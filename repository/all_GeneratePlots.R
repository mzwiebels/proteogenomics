...
DefineChromSizes = function(species){
  if(species=="Human"){
    chrom.sizes = c(248956422,242193529,198295559,190214555,181538259,170805979,159345973,145138636,138394717,133797422,135086622,
...
SetVariableNames = function(method){
  if(method=="Copywriter"){
    ...
    Chromosome <<- "Chrom"
  } else if(method=="LOH_raw"){
    Start <<- "Pos"
    CopyNumber <<- "Tumor_Freq"
    Chromosome <<- "Chrom"
  } else if(method=="LOH_Germline"){
    Start <<- "Pos"
...
ProcessSegmentData = function(segmentdata="",chrom.sizes=chrom.sizes,method=""){
  Outlist = list()
  FirstPosition = c()
  if(method!="aCGH") segmentdata = read.table(segmentdata,header=T,sep="\t")
...
SetYAxis = function(y_axis){
  if(y_axis=="CNV_5"){
    ...
    y_output <<- ".5."
    yborder <<- 5
  } else if(y_axis=="CNV_2"){
    ylim <<- c(-2.5,2.5)
    ypos <<- seq(-2,2,1)
    ylabels <<- seq(-2,2,1)
    y_output <<- ".2."
    yborder <<- 2
  } else if(y_axis=="LOH" || y_axis=="LOH_Germline" || y_axis=="LOH_raw"){
     ylim <<- c(-0.25,1.25)
     ypos <<- c(0,0.5,1)
     ylabels <<- c(0,0.5,1)
     y_output <<- "."
...
plotGlobalRatioProfile = function(cn=cn,ChromBorders=ChromBorders,cnSeg="",samplename="",method="",toolname="",normalization="",y_axis="",Transparency=20, Cex=0.2,outformat=""){
  ...
  if(normalization != "") normalization=paste0(".",normalization)
  
  if(outformat=="emf"){
    emf(paste0(samplename,".",method,".",toolname,normalization,y_output,"emf"),bg="white",width=25,height=10)
  } else if(outformat=="pdf"){
    pdf(paste0(samplename,".",method,".",toolname,normalization,y_output,"pdf"),width=25,height=10)
  }
    par(mar=c(4, 4, 0, 0))
    if(toolname=="CNVKit"){cn = cn[c(TRUE,rep(FALSE,9)), ]}
    plot(cn$position,cn$copy,pch=20,cex=Cex,ylim=ylim,xaxt="n",yaxt="n",bty="n",col=paste("#000000",Transparency,sep=""),yaxs="i",ylab="",xlab="",yaxt="n")
    ...
    if(method=="CNV") segments(cnSeg$start,cnSeg$copy,cnSeg$stop,cnSeg$copy,col="#FF4D4D",lwd=4)
    ChromBordersReduced = ChromBorders[-c(1,LastEntry)]
    if(method == "CNV") segments(ChromBordersReduced,-yborder,ChromBordersReduced,yborder,lty=3,col="grey40",lwd=0.9)
    if(method =="LOH" || method=="LOH_Germline" || method=="LOH_raw") segments(ChromBordersReduced,0,ChromBordersReduced,1,lty=3,col="grey40",lwd=0.9)
  dev.off()
}
...
plotChromosomalRatioProfile = function(cn=cn,chrom.sizes,cnSeg="",samplename="",chromosome="",method="",toolname="",normalization="",y_axis="",SliceStart="",SliceStop="",Transparency=70, Cex=0.3,outformat=""){
  ...
  if((SliceStart!="") & (SliceStop!="")) cn = cn[cn[,Start]>=SliceStart & cn[,Start]<=SliceStop,]
  if((SliceStart=="") & (SliceStop=="")){
    SliceStart=0
    SliceStop=chrom.sizes[chromosome]
  }
  if(normalization != "") normalization=paste(".",normalization,sep="")
