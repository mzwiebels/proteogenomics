...
if [ $input_format = 'Strelka' ]; then
	normalid=NORMAL
	tumorid=TUMOR
elif [ $input_format = 'Mutect2' ] || [ $input_format = 'Manta' ]; then
	normalid=Normal
	tumorid=Tumor
fi
...
if [ $runmode = 'MS' ]; then
	...
	rm $name/results/$input_format/$name.$input_format.vep.vcf_warnings.txt
fi
...
if [ $input_format = 'Mutect2' ]; then
	...
	(
		...
		if [ $type = 'Normal' ]; then
			awk 'BEGIN{FS=OFS="\t"} NR>2{print $0,$NF=0}' $name/results/$input_format/$name.$type.$input_format.vep.maf >> $name/results/$input_format/$name.$type.$input_format.vep.tmp
		elif [ $type = 'Tumor' ]; then
		...
		rm $name/results/$input_format/$name.$type.$input_format.vep.vcf_warnings.txt
	) &
	done
