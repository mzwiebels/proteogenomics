...
usage()
{
	...
	echo "	-s, --species            Sample species, Human or Mouse. Defaults to Human."
...
while [ "$1" != "" ]; do case $1 in
	...
	-qc|--quality_control) shift;quality_control="$1";;
...
if [ -z $config_file ]; then
	config_file=$(whereis MoCaSeq.sh | awk '{print $2}' | sed 's/\/[^\/]*$//')/config.sh
fi
temp_dir=$name"_temp"

if [[ -z $fastq_normal_1 || $fastq_normal_1 = '""' ]] && [[ -z $fastq_normal_2 || $fastq_normal_2 = '""' ]] && [ ! -z $fastq_tumor_1 ] && [ ! -z $fastq_tumor_2 ] && [ -z $bam_normal ] && [ -z $bam_tumor ]; then
	runmode="SS"
	types="Tumor"
elif [ ! -z $fastq_normal_1 ] && [ ! -z $fastq_normal_2 ] && [[ -z $fastq_tumor_1 || $fastq_tumor_1 = '""' ]] && [[ -z $fastq_tumor_2 || $fastq_tumor_2 = '""' ]] && [ -z $bam_normal ] && [ -z $bam_tumor ]; then
	runmode="SS"
	types="Normal"
...
echo '---- Starting Cancer Genome Analysis ----'
...
if [ -z $genome_dir/GetReferenceData.txt ]; then
	...
	sh $repository_dir/Preparation_GetReferenceData.sh $config_file $temp_dir $species $version
elif ! grep -Fxq "DONE" $genome_dir/GetReferenceData.txt; then
	echo '---- Reference files not found - Files will be downloaded ----' | tee -a $name/results/QC/$name.report.txt
	echo -e "$(date) \t timestamp: $(date +%s)" | tee -a $name/results/QC/$name.report.txt
...
echo '---- Starting Cancer Genome Analysis ----' | tee -a $name/results/QC/$name.report.txt
...
if [ $repeat_mapping = "yes" ]; then
	...
	for type in $types; do	
		...
		trimmomatic PE -threads $threads -$phred \
			...
			SLIDINGWINDOW:10:25 \
			ILLUMINACLIP:/fs/pool/pool-mann-projects/MaxZ/conda_envs/genomeseq/share/trimmomatic/adapters/TruSeq3-PE-2.fa:2:30:10 & PIDS="$PIDS $!"
	done
	...
	for type in $types; do
		...
		sambamba markdup --tmpdir $temp_dir \
			--t $threads \
...
sh $repository_dir/SV_MantaPostprocessing.sh $name $config_file $runmode $types
...
if [ $runmode = "MS" ]; then
	...
	sh $repository_dir/SNV_StrelkaPostprocessing.sh $name $species $config_file $filtering
	Rscript $repository_dir/SNV_SelectOutput.R -n $name -i Strelka -s $species -c $CGC_file -r $TruSight_file
fi
...
if [ $Mutect2 = 'yes' ] && [ $runmode = "MS" ]; then
	...
	gatk --java-options '-Xmx256G' Mutect2 \
	...
	sh $repository_dir/SNV_Mutect2Postprocessing.sh $name $species $config_file $filtering

	Rscript $repository_dir/SNV_SelectOutput.R -n $name -i Mutect2 -s $species -c $CGC_file -r $TruSight_file
	#Rscript $repository_dir/SNV_Signatures.R -n $name -s $species
fi
...
if [ $Mutect2 = 'yes' ]; then
	...
	for type in $types; do
		...
		sh $repository_dir/SNV_Mutect2PostprocessingSS.sh $name $species $config_file $type $filtering $artefact_type
		Rscript $repository_dir/SNV_SelectOutputSS.R -n $name -t $type -s $species -c $CGC_file -r $TruSight_file
	done
fi
...
if [ $Mutect2 = 'yes' ] && [ $runmode = "MS" ]; then
	...
	Rscript $repository_dir/LOH_GenerateVariantTable.R -n $name -s $species -f $genome_file -r $repository_dir
	Rscript $repository_dir/LOH_MakePlots.R -n $name -s $species -r $repository_dir
fi
...
if [ $sequencing_type = 'WES' ]; then
	...
	Rscript $repository_dir/CNV_RunCopywriter.R -n $name -s $species -t $threads -m $runmode -g $genome_dir -c $centromere_file -v $varregions_file -r $resolution -p ${types%% *}

	echo '---- Export raw data and re-normalize using Mode ----' | tee -a $name/results/QC/$name.report.txt
	echo -e "$(date) \t timestamp: $(date +%s)" | tee -a $name/results/QC/$name.report.txt

	Rscript $repository_dir/CNV_CopywriterGetRawData.R -n $name -r $runmode -t ${types%% *}

	echo '---- Plot CNV-profiles ----' | tee -a $name/results/QC/$name.report.txt
	echo -e "$(date) \t timestamp: $(date +%s)" | tee -a $name/results/QC/$name.report.txt

	Rscript $repository_dir/CNV_PlotCopywriter.R -n $name -s $species -r $repository_dir -m Mode
	Rscript $repository_dir/CNV_MapSegmentsToGenes.R -n $name -s $species -g $gencode_file_genes -m Copywriter -r $resolution -c $CGC_file -t $TruSight_file
fi

if [ $runmode = "MS" ]; then
	echo '---- Run HMMCopy (bin-size '$resolution') ----' | tee -a $name/results/QC/$name.report.txt
	echo -e "$(date) \t timestamp: $(date +%s)" | tee -a $name/results/QC/$name.report.txt
	...
	Rscript $repository_dir/CNV_PlotHMMCopy.R -n $name -s $species -r $repository_dir -t $sequencing_type -o $resolution -m $mapWig_file -g $gcWig_file -c $centromere_file -v $varregions_file
	Rscript $repository_dir/CNV_MapSegmentsToGenes.R -n $name -s $species -g $gencode_file_genes -m HMMCopy -r $resolution -c $CGC_file -t $TruSight_file
fi
...
if [ $runmode = "MS" ] && [ $sequencing_type = 'WGS' ]; then
	...
	Rscript $repository_dir/CNV_PlotHMMCopy.R -n $name -s $species -r $repository_dir -t $sequencing_type -o 1000 -m $mapWig_file -g $gcWig_file -c $centromere_file -v $varregions_file
	Rscript $repository_dir/CNV_MapSegmentsToGenes.R -n $name -s $species -g $gencode_file_genes -m HMMCopy -r 1000 -c $CGC_file -t $TruSight_file
fi
...
if [ $Titan = "yes" ]; then
	...
	Rscript $repository_dir/all_PrepareTitanCNA.R -n $name -s $species -r $repository_dir -t $sequencing_type -o $resolution -m $mapWig_file -g $gcWig_file -e $exons_file
	sh $repository_dir/all_RunTitanCNA.sh $name $repository_dir $threads $sequencing_type
	Rscript $repository_dir/LOH_MapSegmentsToGenes.R -n $name -s $species -g $gencode_file_genes -c $CGC_file
fi
...
if [ $sequencing_type = 'WGS' ] && [ $Delly = 'yes' ] && [ $runmode = "MS" ]; then
	...
	bcftools view $name/results/Delly/$name.pre.bcf > $name/results/Delly/$name.pre.vcf
fi
	...
	for chr in $( seq $chromosomes ); do
		if [ $(Rscript $repository_dir/Chromothripsis_RearrangementCounter.R -i $name/results/Delly/$name.breakpoints.filtered.tab -c $chr) -ge 4 ]; then
			...
			Rscript $repository_dir/Chromothripsis_SimulateCopyNumberStates.R \
				-i $name/results/Delly/$name.breakpoints.filtered.tab \
				-o $species -c $chr -n $name -s 1000 -a 1000 -f $format -v 1
			...
			Rscript $repository_dir/Chromothripsis_PlotLOHPattern.R \
				...
				-o $species -c $chr -n $name -f $format
